...
@LastEditTime: 2019-10-13 06:45:38
'''
# coding=utf-8
'''
...
class BCP():
    def __init__(self, secparam=1024, param=None):
        if param:
            ...
            self.p, self.q = randomPrime(
                int(secparam/2), True), randomPrime(int(secparam/2), True)
            self.pp = (self.p - 1)/2
            self.qq = (self.q - 1)/2
            ...
            while True:  # choose a good N
                if bitsize(self.N) == secparam and len(int2Bytes(self.N)) == secparam/8 and ord(int2Bytes(self.N)[0]) & 128 != 0:
                    break
                self.p, self.q = randomPrime(
                    secparam/2, True), randomPrime(secparam/2, True)
                self.pp = (self.p - 1)/2
                self.qq = (self.q - 1)/2
                self.N = self.p * self.q
            self.N2 = self.N**2
...
if __name__ == "__main__":
    ...
    pk, sk = bcp.KeyGen()
    x1 = -1
    ciphertext = bcp.Encrypt(pk, x1)
    plaintext = bcp.Decrypt(ciphertext, sk)
    print plaintext
    print "------------------"
    print param.N
